...
/* http_params is declared in ../inc/utils.h */
...
int main (int argc, char* argv[]){
    ...
    while(1){
        ...
        http_params* http = (http_params*) malloc(sizeof(http_params));

        if(network(buffer, clientsock, http) < 0){
...
int network(char* input, int clientsock, http_params* http){
    //parse request
    int input_length = parseRequest(input, http->method, http->request, http->version);
    ...
    int filelen;
    filelen = fileSize(http->request);  //create char array for the file buffer

    char* Contents = (char*)malloc(sizeof(char)* filelen);
    char* headers = (char*)malloc(sizeof(char) * (filelen + BUFFER_LEN));
    headers[0] = '\0';
    int output_length = response_builder(http->request, headers, Contents,  filelen);
